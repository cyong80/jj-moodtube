...
/** 감정 분석 결과 JSON 형식 (Gemini 프롬프트 공통) */
const MOOD_ANALYSIS_JSON_FORMAT =
  "{ 'mood': 'string', 'description': 'string', 'searchQuery': [{'title': '곡명', 'artist': '아티스트명'}] }";

/** searchQuery 다양화 가이드라인 (공통) */
const SEARCH_QUERY_GUIDELINES = `searchQuery는 18~20곡의 배열로, **가능한 한 다양한** 노래를 추천해주세요:
...
/** 맥락 고려 사항 (날씨, 계절, 기념일 등) */
const CONTEXT_CONSIDERATIONS =
  "분석 시 현재 날씨, 계절, 절기(입춘, 경칩, 청명 등), 기념일(크리스마스, 발렌타인데이, 추석 등)도 함께 고려하세요.";

/** 로그인 사용자의 좋아요 곡 목록 (추천 프롬프트용, 최대 30곡) */
async function getLikedTracksForRecommendation(): Promise<
...
> {
  ...
  const likes = await prisma.moodTubeLike.findMany({
    ...
    include: { youtubeSearchCache: true },
  ...
  return likes
    ...
    .filter(
      (c): c is NonNullable<typeof c> =>
        c != null && c.youtubeId != null && c.title != null
    )
    .map((c) => ({
      title: c.title!,
      artist: c.channel ?? "알 수 없음",
    }));
}

/** 좋아요 곡 참고용 프롬프트 섹션 */
function buildLikedSongsPromptSection(
  ...
  const list = likedTracks
    ...
    .join("\n");
  return `
이 사용자가 좋아요한 노래를 참고하여, 비슷한 장르·스타일·아티스트의 추천을 searchQuery에 포함해주세요.
단, 아래 목록에 있는 곡과 동일한 곡(title·artist가 같은 곡)은 searchQuery에 포함하지 마세요.
좋아요한 노래 목록:
${list}
`;
}
...
export async function getMoodPlaylist(base64Image: string) {
  try {
    ...
    const likedTracks = await getLikedTracksForRecommendation();
    const likedSongsSection = buildLikedSongsPromptSection(likedTracks);
    ...
    const analysis = await analyzeWithGemini([
      {
        ...
        parts: [
          {
            text: `당신은 전문 음악 큐레이터입니다. 이미지를 분석하고 반드시 다음 형식의 JSON 객체를 반환하세요: ${MOOD_ANALYSIS_JSON_FORMAT}`,
          },
          { inlineData: { data: imageData, mimeType: "image/jpeg" } },
          { text: "이 사람의 감정과 주변 환경의 분위기를 분석해주세요." },
          {
            text: `${CONTEXT_CONSIDERATIONS} 인물사진은 인물의 기분, 그 외는 이미지 내용을 고려하세요.${likedSongsSection}
    ...
    const analysis = await analyzeWithGemini([
      {
        ...
        parts: [
          {
            text: `당신은 전문 음악 큐레이터입니다. 사용자의 음성/텍스트를 분석하고 반드시 다음 형식의 JSON 객체를 반환하세요: ${MOOD_ANALYSIS_JSON_FORMAT}

사용자가 말한 내용:
"${text}"

위 내용에서 사용자의 기분, 감정, 원하는 분위기, 상황 등을 분석해주세요. ${CONTEXT_CONSIDERATIONS}${likedSongsSection}
...
반드시 다음 형식의 JSON 객체만 반환하세요: ${MOOD_ANALYSIS_JSON_FORMAT}
description에는 말한 내용과 목소리에서 느껴진 기분을 함께 설명해주세요.${likedSongsSection}
